...
function commitToGithub() {
  ...
  childProcess.execSync(
    `git clone --single-branch --branch data \"https://x-access-token:$API_TOKEN_GITHUB@github.com/SiongSng/Rapid-Antigen-Test-Taiwan-Map.git\" \"${cloneDir}\"`
  );
  ...
  const needsCommit: boolean =
    childProcess.execSync(`${cdCommand} git status --porcelain`, {
      encoding: "utf8",
    }).length > 0;
  ...
  if (needsCommit) {
    ...
    childProcess.execSync(
      `${cdCommand} git commit --message \"Auto update data\"`
    );
